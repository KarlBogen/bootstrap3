  ...
  if (isset($_SESSION['customer_id'])) {
    ...
    if (xtc_db_num_rows($orders_query) > 0) {
      while ($orders = xtc_db_fetch_array($orders_query)) {
        $customer_orders_array[] = array(
          'PRODUCTS_LINK' => '<a class="btn btn-default" href="' . xtc_href_link(FILENAME_PRODUCT_INFO, xtc_product_link($orders['products_id'], $orders['products_name'])) . '" style="white-space:normal;">' . $orders['products_name'] . '</a>',
          'ORDER_LINK' => '<a class="btn btn-success" href="' . xtc_href_link(basename($PHP_SELF), xtc_get_all_get_params(array('action')) . 'action=add_order_product&order_id='.$orders['orders_id'].'&id='.$orders['orders_products_id']) . '"><span class="glyphicon glyphicon-shopping-cart"></span></a>',
        );
      }
    }
