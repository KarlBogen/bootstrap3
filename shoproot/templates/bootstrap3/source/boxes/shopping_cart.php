  ...
  if ($_SESSION['cart']->count_contents() > 0) {
    ...
    if (strpos($PHP_SELF, FILENAME_LOGOFF) === false) {
      ...
      for ($i = 0, $n = count($products); $i < $n; $i++) {
        $del_button = '<a href="' . xtc_href_link(basename($PHP_SELF), xtc_get_all_get_params(array('action', 'box', 'prd_id')).'action=remove_product&box=cart&prd_id=' . $products[$i]['id'], 'NONSSL') . '" title="' . IMAGE_BUTTON_DELETE . '"><span class="glyphicon glyphicon-trash"></span></a>';
